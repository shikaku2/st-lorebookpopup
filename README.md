...
| Hide always-on entries | On | Filter out constant (non-vectorized) entries that are always active |
| Log Qdrant Memory retrievals | On | *(shown only when st-qdrant-memory is installed)* Keep a rolling log of retrieved memories and include them in the click-through modal |
